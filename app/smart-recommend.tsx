...
export default function SmartRecommendScreen() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['smart-recommend', lat, lng, mode],
    queryFn: () => placeService.smartRecommend(lat, lng, mode),
    staleTime: 5 * 60 * 1000,
  });
    ...
    <ScreenTransition>
      <LinearGradient colors={['#F5F3FF', '#FFFFFF']} style={{ flex: 1 }}>
        ...
        <View style={styles.toggleWrap}>
          ...
          <TouchableOpacity
            style={[styles.toggleBtn, mode === 'wide' && styles.toggleBtnActive]}
            onPress={() => setMode('wide')}
            activeOpacity={0.8}
          >
            {mode === 'wide' && (
              <LinearGradient colors={['#7C3AED', '#A855F7']} style={StyleSheet.absoluteFill} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} />
            )}
            <Globe size={14} color={mode === 'wide' ? '#fff' : '#7C3AED'} />
            <Text style={[styles.toggleText, mode === 'wide' && styles.toggleTextActive]}>서울 전체</Text>
          </TouchableOpacity>
        </View>
              ...
              >
                ...
                {mode === 'wide' && (
                  <View style={[styles.contextBadge, { backgroundColor: '#FDF2F8' }]}>
                    <Text style={[styles.contextText, { color: '#9D174D' }]}>🏙 서울 전체</Text>
                  </View>
                )}
              </Animated.View>
